...
{/* min-version: 2.1.205 */}일반 `claude` 세션 내에서 프롬프트 푸터의 `←` 힌트는 `← 2 agents`와 같이 입력을 기다리는 백그라운드 에이전트의 수를 세고, 입력이 필요한 에이전트가 없을 때 `← for agents`로 돌아갑니다. 99 이상의 개수는 `99+`로 표시됩니다. 개수는 터미널이 포커스되어 있는 동안 약 10초마다 새로 고쳐지고 포커스가 돌아올 때 즉시 새로 고쳐집니다. [`prefersReducedMotion` 설정](/docs/ko/settings#available-settings)이 켜져 있지 않으면 이동할 때와 에이전트가 완료될 때 색상이 잠깐 변하며, [화면 읽기 모드](/docs/ko/accessibility)에서는 숨겨집니다. [Amazon Bedrock, Google Cloud의 Agent Platform, Microsoft Foundry](/docs/ko/third-party-integrations)에서는 힌트가 개수 없이 일반 `← for agents` 형태로 유지됩니다. Claude Code v2.1.205 이상이 필요합니다.
...
이름은 해당 세션에서 [`/color`](/docs/ko/commands)로 설정된 색상으로 표시됩니다. {/* min-version: 2.1.199 */}v2.1.199부터 색상은 `←` 또는 `/background`로 [세션을 백그라운드로 보낼](#from-inside-a-session) 때 유지됩니다.
...
행은 대화 기록이 없는 새로운 세션에서도 생성되므로 `→`는 이를 반환합니다. {/* max-version: 2.1.202 */}v2.1.203 이전에는 해당 행이 유일한 경우 에이전트 뷰가 아래에 온보딩 힌트를 표시했습니다.
...
800자보다 길거나 2줄 이상인 붙여넣은 텍스트는 `[Pasted text #N]` 자리 표시자로 축소되어 입력이 한 줄로 유지됩니다. 디스패치할 때 전체 텍스트가 전송됩니다. {/* min-version: 2.1.207 */}디스패치하기 전에 축소된 텍스트를 검토하거나 편집하려면 동일한 텍스트를 다시 붙여넣으면 자리 표시자가 입력으로 다시 확장됩니다. 너비가 최소 90열인 터미널에서 붙여넣은 후 몇 초 동안 입력 아래에 `paste again to expand` 알림이 나타납니다. v2.1.207 이전에는 동일한 텍스트를 다시 붙여넣으면 첫 번째 자리 표시자를 확장하는 대신 두 번째 자리 표시자가 추가되었습니다.
...
* {/* min-version: 2.1.198 */}v2.1.198부터 `/login`은 세션에 연결하지 않고 다시 로그인할 수 있도록 로그인 대화 상자를 엽니다

스킬, 사용자 정의 명령 및 `/init`과 같은 프롬프트 확장 기본 제공 명령은 새로운 백그라운드 세션으로 첫 번째 프롬프트로 전송됩니다. 다른 기본 제공 명령은 대신 `세션에 연결하여 실행` 힌트를 표시합니다. {/* min-version: 2.1.203 */}입력한 모든 내용은 힌트 옆의 입력에 유지되므로 편집할 수 있습니다. v2.1.203 이전에는 힌트가 입력을 지웠고 입력한 텍스트가 손실되었습니다.
...
* 상위 디렉토리에서 `claude agents`를 열고 프롬프트에서 `@<repo>`로 하위 저장소를 언급합니다. `@`를 입력하면 이러한 대상이 나열됩니다:
  ...
  * 이미 목록에 세션이 있는 모든 디렉토리
  ...
  이름에 공백이 포함된 디렉토리는 나열되지 않습니다. {/* min-version: 2.1.203 */}v2.1.203 이전에는 등록된 worktree가 나열되지 않았으므로 worktree로 디스패치하려면 해당 worktree의 디렉토리에서 `claude --bg`를 실행해야 했습니다.
...
프롬프트는 위치 인수이며 `-p` 값이 아닙니다. {/* min-version: 2.1.198 */}v2.1.198부터 `--bg`를 `-p` 또는 `--print`와 결합하면 세션이 생성되기 전에 오류로 거부됩니다. `--print`는 `claude agents`가 연결하는 대화형 세션을 시작하지 않기 때문입니다.
...
`CLAUDE_CODE_USE_BEDROCK` 또는 `CLAUDE_CODE_USE_VERTEX`와 같은 클라우드 공급자 선택 및 `ANTHROPIC_DEFAULT_*_MODEL` 별칭은 세션을 디스패치한 셸을 따릅니다. {/* min-version: 2.1.206 */}해당 셸에서 [`CLAUDE_CODE_EXTRA_BODY`](/docs/ko/env-vars) 요청 본문 재정의를 내보내면 동일한 방식으로 세션에 도달합니다. v2.1.206 이전에는 백그라운드 워커가 셸 내보낸 `CLAUDE_CODE_EXTRA_BODY`를 무시했습니다.
...
{/* min-version: 2.1.198 */}v2.1.198부터 인계는 세 가지 모두를 포함합니다. v2.1.198 이전에는 셸 명령과 워크플로우만 포함했으므로 백그라운드 서브에이전트는 프로세스와 함께 중지되었고 다음 깨어날 때 실패한 것으로 보고되었습니다.
...
세션은 버전 불일치를 그대로 유지합니다. 이전 Claude Code 버전이 세션의 `state.json`을 업데이트할 때 인식하지 못하는 필드를 보존하고 세션을 나열된 상태로 유지합니다. {/* min-version: 2.1.200 */}`roster.json`의 세션 목록은 동일한 규칙을 따릅니다. 이전 버전이 다시 쓸 때 최신 버전이 작성한 필드를 보존하므로 최신 버전으로 시작한 세션은 도달 가능한 상태로 유지되고 감독자가 다시 시작한 후에도 입력을 계속 받습니다. v2.1.200 이전에는 이전 버전이 다시 쓸 때 해당 필드를 삭제할 수 있었습니다.
