...
* **`Voice connection failed`**: 연결이 실패했기 때문에 녹음이 전사 서비스에 도달하지 않았습니다. 네트워크를 확인하고 다시 시도하세요. {/* min-version: 2.1.200 */}오디오를 캡처하지 않는 녹음은 이 메시지 대신 `No audio detected from microphone`을 보고합니다. v2.1.200 이전에는 침묵한 마이크가 연결 실패를 보고할 수 있었으며, 이는 실제 문제가 입력 장치일 때 네트워크 문제를 시사했습니다.
