...
이 페이지는 CLI 및 SDK 유지 관리자를 위한 것입니다. 플러그인 설치를 찾고 있다면 [플러그인 발견 및 설치](/ko/discover-plugins)를 참조하세요.
...
힌트 프로토콜은 공식 Anthropic 마켓플레이스 `claude-plugins-official`에 나열된 플러그인에 대해서만 적용됩니다. Anthropic은 자신의 재량에 따라 해당 마켓플레이스를 큐레이션하며, 앱 내 제출 양식은 플러그인을 [커뮤니티 마켓플레이스](/ko/plugins#submit-your-plugin-to-the-community-marketplace)에 추가합니다. 힌트 프로토콜은 이를 확인하지 않습니다. Anthropic 파트너 담당자와 함께 작업 중인 경우 그들에게 연락하여 공식 마켓플레이스 목록을 조정하세요.

<h2 id="see-also">
  참고 항목
</h2>

* [플러그인 만들기](/ko/plugins): CLI가 권장하는 플러그인 빌드
* [플러그인 마켓플레이스 만들기 및 배포](/ko/plugin-marketplaces): 공식 마켓플레이스 외부에서 플러그인 호스팅
* [환경 변수](/ko/env-vars): `CLAUDECODE` 및 관련 변수에 대한 전체 참조
