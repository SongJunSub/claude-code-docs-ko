...
Claude Code는 제한에 접근할 때 자동으로 압축하므로 컨텍스트 윈도우가 가득 차도 세션이 끝나지 않습니다. 자동 패스는 타임라인의 `/compact` 단계와 동일한 방식으로 작동합니다. [컨텍스트가 가득 찰 때](/ko/how-claude-code-works#when-context-fills-up)에서 보존되는 항목을 참조하세요.
...
* **대용량 읽기 위임**: 연구를 [서브에이전트](/ko/sub-agents)에 보내 파일 콘텐츠가 사용자의 컨텍스트 윈도우가 아닌 서브에이전트의 컨텍스트 윈도우에 유지되도록 하세요.

더 작은 대화보다 더 큰 윈도우가 필요한 경우 Fable 5, Sonnet 5, Opus 4.6 이상, Sonnet 4.6은 100만 토큰 컨텍스트 윈도우를 지원합니다. 플랜별 가용성 및 `[1m]` 모델 변형을 선택하는 방법은 [확장 컨텍스트](/ko/model-config#extended-context)를 참조하세요. Sonnet 5는 선택할 `[1m]` 변형 없이 1M으로 실행됩니다. 자동 압축 임계값 및 LLM 게이트웨이 예외에 대해서는 [Sonnet 5 컨텍스트 윈도우](/ko/model-config#sonnet-5-context-window)를 참조하세요. 압축은 더 큰 제한에서도 동일한 방식으로 작동합니다.
