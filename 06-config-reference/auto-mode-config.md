...
* **컨텍스트 슬롯**: 조직, 스택 및 보안 태세를 설명하여 분류기가 컨텍스트의 다른 규칙을 읽을 수 있도록 합니다. 다른 두 가지 종류와 달리 컨텍스트 슬롯은 이를 대상으로 하는 자체 규칙이 없습니다. 각각은 `None configured`로 기본 설정되거나 옆에 명명된 보수적 가정으로 기본 설정됩니다:
  ...
  * **클라우드 제공자**
  * **저장소 가시성**: 원격 호스트 및 이름이 다르게 표시하지 않는 한 저장소는 비공개로 가정됩니다. {/* min-version: 2.1.200 */}또는 대화 초반에 분류기가 읽는 가시성 확인이 공개임을 보여줍니다. 분류기는 사용자의 메시지와 Claude가 실행하는 명령을 읽으며, 그 출력은 읽지 않으므로, 증거는 저장소를 공개로 명명하는 사용자의 메시지와 같이 읽을 수 있는 것이어야 합니다. `gh repo view`의 출력만으로는 도달하지 않습니다. 트랜스크립트 증거 확인은 Claude Code v2.1.200 이상이 필요합니다.
  ...
  * **데이터 보존 / 기밀 해제**
* **신뢰 슬롯**: 분류기가 경계 내부로 취급하는 것을 명명합니다. 슬롯은 신뢰할 수 있는 저장소, 소스 제어, 신뢰할 수 있는 내부 도메인, 신뢰할 수 있는 클라우드 버킷, 주요 내부 서비스 및 내부 패키지 레지스트리입니다. 저장소 및 소스 제어 항목은 기본적으로 작업 저장소와 그 구성된 원격 저장소로 설정됩니다. 다른 모든 신뢰 슬롯은 기본적으로 `None configured`로 설정되므로, 추가할 때까지 다른 것은 신뢰되지 않습니다. {/* min-version: 2.1.203 */}저장소의 가시성은 기밀 자료만 범위를 지정합니다. 비공개 저장소는 기밀 자료의 허용 가능한 대상이지만, 저장소를 비공개로 만드는 것은 비밀, 개인 또는 위탁 데이터를 그 안에 지우지 않으며, 분류기는 작업 저장소 외부에서 이식되거나 재지정되거나 처음 읽은 콘텐츠를 해당 저장소 자체의 작업으로 취급하지 않습니다. 이 범위 지정은 Claude Code v2.1.203 이상이 필요합니다.
...
* **민감한 데이터 위치 및 대상**: 개인 데이터, 기밀 비즈니스 데이터, 자격 증명, 규제 데이터 또는 유사하게 민감한 자료를 보유하는 버킷, 데이터베이스 또는 경로, 그리고 각 위치의 데이터가 공유될 수 있는 대상이므로, 분류기가 콘텐츠에서 추측하는 대신 해당 위치를 보호합니다. {/* min-version: 2.1.195 */}{/* max-version: 2.1.197 */}Claude Code v2.1.195부터 v2.1.197까지는 이 항목을 PII / 규제 데이터 위치로 명명하고 대상 차원 없이 개인 또는 규제 데이터를 보유하는 위치만 포함합니다
...
{/* min-version: 2.1.208 */}파이프를 `jq`로 통과하지 않고 한 규칙의 전체 표현을 읽으려면 규칙 레이블의 시작 부분(예: `claude auto-mode defaults --label 'Git Destructive'`)과 함께 `--label`을 전달합니다. 일치는 각 규칙의 레이블에 대한 대소문자를 구분하지 않는 접두사이며, 일치하는 항목이 없는 섹션은 빈 목록으로 인쇄됩니다. Claude Code v2.1.208 이상이 필요합니다.
