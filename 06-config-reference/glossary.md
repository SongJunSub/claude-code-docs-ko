...
백그라운드에서 별도의 분류기 모델이 각 작업을 검토하는 [권한 모드](#permission-mode)이며, 승인 프롬프트를 표시하는 대신입니다. 분류기는 범위 확대, 신뢰할 수 없는 인프라 및 [프롬프트 주입](#prompt-injection)을 차단합니다. 도구 결과를 보지 않으므로 주입된 지침이 결정에 영향을 미칠 수 없습니다. Auto mode는 연구 미리보기입니다.
