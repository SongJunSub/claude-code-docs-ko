...
`--dangerously-skip-permissions`는 명시적 [요청 규칙](/ko/permissions#manage-permissions)을 제외한 작업별 검토를 제거하므로 격리 경계가 Claude가 할 수 있는 작업을 제한하는 유일한 것입니다. 항상 컨테이너, VM 또는 [샌드박스 런타임](#sandbox-runtime) 내에서 실행하여 파일 도구, MCP 서버, 훅도 경계 내에 있도록 하세요.
